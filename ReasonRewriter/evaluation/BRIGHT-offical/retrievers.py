...
@torch.no_grad()
def retrieval_sf_qwen_e5(queries, query_ids, documents, doc_ids, task, model_id, instructions, cache_dir, excluded_ids,
                         long_context, **kwargs):
    if model_id == 'sf':
    ...
    for start_idx in trange(0, len(documents), batch_size):
        ...
        batch_dict = tokenizer(documents[start_idx:start_idx + batch_size], max_length=max_length, padding=True,
...
def retrieval_bm25(queries, query_ids, documents, doc_ids, excluded_ids, long_context, **kwargs):
    ...
    class SparseMatrixSimilarityNew(SparseMatrixSimilarity):
        ...
        def get_similarities(self, query):
            ...
            query_csc = query.tocsc()  # Convert once to CSC format
            result = self.index * query_csc  # Sparse matrix multiplication

            # Post-process result
            if result.shape[1] == 1 and not is_corpus:
                return result.toarray().flatten()  # Return 1D array for single document
            elif self.maintain_sparsity:
                return result.T  # Return sparse matrix
            else:
                return result.toarray().T  # Return dense matrix
    ...
    bm25_index = SparseMatrixSimilarityNew(bm25_corpus, num_docs=len(corpus), num_terms=len(dictionary),
                                           normalize_queries=False, normalize_documents=False)
    all_scores = {}
    bar = tqdm(queries, desc="BM25 retrieval")
    for now_idx, (query_id, query) in enumerate(zip(query_ids, queries)):
        bar.update(1)
        if isinstance(query, str):
            ...
            uids = kwargs.pop('useful_ids', None)
            all_scores[str(query_id)] = {}
            length = len(query)
            weights = 1000
            if uids is not None:
                for uid in uids[now_idx]:
                    all_scores[str(query_id)][uid] = weights
                    weights -= 5
            for weight, q in query.items():
                q = analyzer.analyze(q)
                bm25_query = model[dictionary.doc2bow(q)]
                similarities = bm25_index[bm25_query].tolist()
                for did, s in zip(doc_ids, similarities):
                    if did not in all_scores[str(query_id)]:
                        all_scores[str(query_id)][did] = s * weight / 100
                    else:
                        all_scores[str(query_id)][did] = max(all_scores[str(query_id)][did], s * weight / 100)
            for did in set(excluded_ids[str(query_id)]):
                if did != "N/A":
                    all_scores[str(query_id)].pop(did)
            cur_scores = sorted(all_scores[str(query_id)].items(), key=lambda x: x[1], reverse=True)[:1000]
            ...
            for q in query:
                ...
                for did, s in zip(doc_ids, similarities):
                    if did not in all_scores[str(query_id)]:
                        all_scores[str(query_id)][did] = s / length
                    else:
                        all_scores[str(query_id)][did] += s / length
            for did in set(excluded_ids[str(query_id)]):
                if did != "N/A":
...
@torch.no_grad()
def retrieval_sbert_bge(queries, query_ids, documents, doc_ids, task, instructions, model_id, cache_dir, excluded_ids,
                        long_context, **kwargs):
    if model_id == 'bge':
...
@torch.no_grad()
def retrieval_m3(queries, query_ids, documents, doc_ids, task, instructions, model_id, cache_dir, excluded_ids,
                        long_context, **kwargs):
    model = FlagModel(
        model_name='/share/project/shared_models/models--BAAI--bge-m3/snapshots/5617a9f61b028005a4858fdac845db406aefb181/',
        use_fp16=True
    )
    cache_dir = '/share/project/chaofan/code/bright-offical/data/old/bge-m3-emb'
    batch_size = kwargs.get('batch_size', 128)
    ...
    if os.path.isfile(cur_cache_file):
        ...
        doc_emb = model.encode(documents, batch_size=batch_size)
        np.save(cur_cache_file, doc_emb)
...
@torch.no_grad()
def retrieval_instructor(queries, query_ids, documents, doc_ids, task, instructions, model_id, cache_dir, excluded_ids,
                         long_context, **kwargs):
    if model_id == 'inst-l':
    ...
    if os.path.isfile(cur_cache_file):
        ...
        doc_embs = model.encode(documents, show_progress_bar=True, batch_size=batch_size, normalize_embeddings=True,
                                prompt=instructions['document'].format(task=task))
...
@torch.no_grad()
def retrieval_grit(queries, query_ids, documents, doc_ids, task, instructions, model_id, cache_dir, excluded_ids,
                   long_context, **kwargs):
...
def retrieval_openai(queries, query_ids, documents, doc_ids, task, model_id, cache_dir, excluded_ids, long_context,
                     **kwargs):
    ...
    for idx in trange(0, len(documents), batch_size):
        cur_cache_file = os.path.join(cache_dir, 'doc_emb', model_id, task, f"long_{long_context}_{batch_size}",
                                      f'{idx}.json')
        if os.path.isfile(cur_cache_file):
            with open(cur_cache_file) as f:
                cur_emb = json.load(f)
        else:
            cur_emb = get_embedding_openai(texts=documents[idx:idx + batch_size], openai_client=openai_client,
                                           tokenizer=tokenizer)
...
def retrieval_cohere(queries, query_ids, documents, doc_ids, task, model_id, cache_dir, excluded_ids, long_context,
                     **kwargs):
    query_emb = []
    doc_emb = []
...
def retrieval_voyage(queries, query_ids, documents, doc_ids, task, model_id, cache_dir, excluded_ids, long_context,
                     **kwargs):
    ...
    for i in trange(0, len(documents), batch_size):
        ...
        while not success:
            ...
            if exec_count > 5:
                ...
                exit(0)
            try:
                cur_emb = voyage_client.embed(cur_texts, model="voyage-large-2-instruct",
...
def retrieval_google(queries, query_ids, documents, doc_ids, task, model_id, cache_dir, excluded_ids, long_context,
                     **kwargs):
    ...
    for start_idx in tqdm(range(0, len(queries), batch_size), desc='embedding'):
        query_emb += get_embedding_google(texts=queries[start_idx:start_idx + batch_size], task='RETRIEVAL_QUERY',
                                          model=model)
    scores = pairwise_cosine_similarity(torch.tensor(query_emb), torch.tensor(doc_emb))
    scores = scores.tolist()
    return get_scores(query_ids=query_ids, doc_ids=doc_ids, scores=scores, excluded_ids=excluded_ids)
