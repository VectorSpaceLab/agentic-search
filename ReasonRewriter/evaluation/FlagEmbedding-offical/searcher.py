    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(list(queries.values())[0], list):
            queries_texts = [[] for _ in range(len(list(queries.values())[0]))]
            for qid, query in queries.items():
                for i in range(len(query)):
                    queries_texts[i].append(query[i])
                queries_ids.append(qid)
            tmp = []
            for e in queries_texts:
                tmp.extend(e)
            queries_texts = tmp
        else:
            for qid, query in queries.items():
        ...
        if isinstance(list(queries.values())[0], list):
            queries_emb = queries_emb.reshape(len(queries), len(list(queries.values())[0])).mean(axis=1)
